...
bool cart_load(const std::string &card_name) {
  ...
  fmt::println("\t Type     : {0:02X} ({1})", ctx.header->type, getROMType());
  fmt::println("\t ROM Size : {} KB", 32 << ctx.header->rom_size);
  fmt::println("\t RAM Size : {0:02X}", ctx.header->ram_size);
  fmt::println("\t LIC Code : {0:02X} ({1})", ctx.header->license_code,
               getCartLicenseName());
  fmt::println("\t ROM Vers : {0:02X}", ctx.header->version);
