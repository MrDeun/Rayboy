...
\subsubsection{Znaczenie architektoniczne magistrali}
...
\subsubsection{Reprezentacja i przechowywanie instrukcji procesora}

Najprostszym podejściem do implementacji zestawu rozkazów procesora jest naiwna metoda polegająca na ręcznym definiowaniu logiki wykonania dla każdego kodu operacji (\textit{opcode}). W takim rozwiązaniu stosuje się zazwyczaj rozbudowaną instrukcję warunkową lub konstrukcję \texttt{switch-case}, w której każdemu kodowi operacji odpowiada bezpośrednia implementacja jego działania.

Podejście to, mimo swojej prostoty koncepcyjnej, prowadzi do szeregu problemów projektowych. Przede wszystkim skutkuje silnym powiązaniem informacji o instrukcji (typ operacji, tryb adresowania, używane rejestry, warunki wykonania) z jej implementacją wykonawczą. W efekcie kod staje się trudny do utrzymania, podatny na błędy oraz mało skalowalny. Dodanie nowej instrukcji lub modyfikacja istniejącej wymaga ingerencji w wiele fragmentów kodu, co zwiększa ryzyko regresji. Ponadto powtarzalność wzorców implementacyjnych utrudnia analizę oraz optymalizację emulatora.

W celu rozwiązania tych problemów zastosowano podejście oparte na rozdzieleniu opisu instrukcji od mechanizmu jej wykonania. Każdy rozkaz procesora reprezentowany jest przez strukturę danych zawierającą komplet informacji niezbędnych do jego obsługi, takich jak:
\begin{itemize}
	\item typ operacji,
...
Struktury te przechowywane są w tablicy indeksowanej bezpośrednio kodem operacji. Dzięki temu etap dekodowania instrukcji sprowadza się do odczytu odpowiedniej struktury na podstawie wartości \texttt{opcode}. Mechanizm wykonania korzysta następnie z tych informacji w sposób uogólniony, delegując właściwą logikę do funkcji przypisanej do typu instrukcji.

Takie podejście wprowadza wyraźną separację odpowiedzialności pomiędzy:
\begin{enumerate}
	\item \textbf{opis semantyki instrukcji} — przechowywany w strukturach danych,
	\item \textbf{mechanizm dekodowania} — wybór instrukcji na podstawie kodu operacji,
	\item \textbf{mechanizm wykonania} — implementacja operacji procesora.
\end{enumerate}

Rozdzielenie danych opisujących instrukcję od jej implementacji przynosi istotne korzyści architektoniczne. Kod staje się bardziej czytelny, łatwiejszy do rozszerzania oraz odporny na błędy wynikające z duplikacji logiki. Ponadto możliwe jest jednolite przetwarzanie różnych klas instrukcji, co upraszcza implementację trybów adresowania oraz mechanizmów debugowania.

Zastosowana reprezentacja stanowi zatem warstwę abstrakcji pomiędzy kodem maszynowym a logiką emulatora, umożliwiając elastyczne i skalowalne odwzorowanie zestawu rozkazów procesora.

\subsubsection{Struktura procesora oraz organizacja cyklu wykonawczego}

Implementacja procesora emulatora opiera się na centralnej strukturze kontekstu wykonania, która przechowuje kompletny stan jednostki CPU oraz dane pomocnicze wykorzystywane podczas dekodowania i realizacji instrukcji. Takie podejście umożliwia jednoznaczne odwzorowanie stanu procesora w dowolnym momencie działania emulatora oraz zapewnia spójny interfejs komunikacji pomiędzy modułami systemu.

\subsubsection{Struktura rejestrów procesora}

Stan architektoniczny procesora przechowywany jest w strukturze \texttt{cpu\_registers}, zawierającej rejestry ogólnego przeznaczenia oraz rejestry sterujące:
\begin{itemize}
	\item rejestry 8-bitowe: \texttt{A, F, B, C, D, E, H, L},
	\item licznik rozkazów \texttt{PC} (Program Counter),
	\item wskaźnik stosu \texttt{SP} (Stack Pointer).
\end{itemize}

Rejestr \texttt{F} pełni funkcję rejestru flag procesora i przechowuje bity statusowe operacji arytmetyczno-logicznych, takie jak:
\begin{itemize}
	\item flaga zera (Z),
	\item flaga odejmowania (N),
	\item flaga półprzeniesienia (H),
	\item flaga przeniesienia (C).
\end{itemize}

\subsubsection{Kontekst wykonania procesora}

Pełny stan wykonawczy przechowywany jest w strukturze \texttt{cpu\_context}. Oprócz rejestrów zawiera ona:
\begin{itemize}
	\item licznik taktów \texttt{ticks} wykorzystywany do synchronizacji z pozostałymi komponentami emulatora,
	\item pole \texttt{fetch\_data} przechowujące operand pobrany z pamięci lub rejestru,
	\item pole \texttt{mem\_destination} określające adres docelowy operacji zapisu,
	\item flagę \texttt{dest\_is\_mem} informującą, czy wynik operacji powinien zostać zapisany do pamięci,
	\item aktualny kod operacji \texttt{op\_code},
	\item wskaźnik na aktualnie wykonywaną instrukcję \texttt{cur\_instruction}.
\end{itemize}

Struktura zawiera również pola sterujące pracą procesora:
\begin{itemize}
	\item globalne zezwolenie na przerwania,
	\item flagę aktywacji przerwań po wykonaniu instrukcji,
	\item stan zatrzymania procesora,
	\item flagi zgłoszeń przerwań oraz rejestr maskujący przerwania.
\end{itemize}

\subsubsection{Inicjalizacja procesora}

Funkcja \texttt{cpu\_init()} odpowiada za ustawienie początkowego stanu procesora zgodnego z oczekiwanym stanem systemu po uruchomieniu. Inicjalizowane są wartości rejestrów, licznik rozkazów, wskaźnik stosu oraz rejestry systemu przerwań. Funkcja ta inicjuje również pamięć operacyjną emulatora.

\subsubsection{Pobieranie i dekodowanie instrukcji}

Funkcja \texttt{fetch\_instruction()} realizuje etap pobrania instrukcji. Kod operacji odczytywany jest z pamięci pod adresem wskazywanym przez licznik rozkazów, po czym następuje jego inkrementacja. Następnie wykonywane jest wyszukanie struktury opisującej instrukcję, zawierającej informacje o jej semantyce oraz trybie adresowania.

\subsubsection{Pobieranie operandów}

Funkcja \texttt{fetch\_data()} odpowiada za odczyt operandów wymaganych przez instrukcję. W zależności od trybu adresowania operandy mogą pochodzić z:
\begin{itemize}
	\item rejestrów procesora,
	\item pamięci operacyjnej,
	\item bezpośrednich wartości zapisanych w kodzie programu.
\end{itemize}

Podczas tego etapu przygotowywane są również informacje o miejscu zapisu wyniku operacji.

\subsubsection{Wykonanie instrukcji}

Funkcja \texttt{execute()} realizuje właściwe wykonanie instrukcji. Na podstawie typu operacji wybierana jest odpowiednia funkcja wykonawcza, której przekazywany jest kontekst procesora. Rozdzielenie logiki wykonawczej od etapu dekodowania umożliwia modularną organizację kodu emulatora.

\subsubsection{Symulacja cykli zegarowych}

Funkcja \texttt{emu\_cycles()} odwzorowuje upływ czasu w systemie. Każdy cykl powoduje:
\begin{itemize}
	\item inkrementację licznika taktów procesora,
	\item aktualizację timera systemowego,
	\item aktualizację układu graficznego,
	\item obsługę kontrolera DMA.
\end{itemize}

Mechanizm ten zapewnia synchronizację CPU z pozostałymi komponentami emulatora.

\subsubsection{Główny cykl pracy procesora}

Funkcja \texttt{cpu\_step()} realizuje pojedynczy krok pracy procesora. Obejmuje ona:
\begin{enumerate}
...
W przypadku zatrzymania procesora wykonywane są jedynie cykle zegarowe do momentu wystąpienia przerwania.

\subsubsection{Dostęp do rejestrów procesora: operacje get/set}

Warstwa dostępu do rejestrów procesora realizuje ujednolicony interfejs odczytu i zapisu zarówno dla rejestrów 8-bitowych, jak i 16-bitowych. Rozwiązanie to abstrahuje fizyczną reprezentację rejestrów w strukturze kontekstu CPU i umożliwia jednolite operowanie na nich przez mechanizmy dekodowania oraz wykonania instrukcji.

\paragraph{Odczyt rejestrów 16-bitowych}

Funkcja \texttt{cpu\_read\_reg(reg\_type rt)} zwraca wartość wskazanego rejestru. Dla rejestrów 8-bitowych operacja sprowadza się do bezpośredniego odczytu pola struktury kontekstu. W przypadku rejestrów złożonych (AF, BC, DE, HL) stosowana jest interpretacja dwóch sąsiadujących rejestrów 8-bitowych jako jednej wartości 16-bitowej.

Zastosowanie funkcji pomocniczej realizującej odwrócenie kolejności bajtów zapewnia zgodność z reprezentacją little-endian stosowaną w systemie.

\paragraph{Zapis rejestrów 16-bitowych}

Funkcja \texttt{cpu\_set\_reg(reg\_type rt, uint16\_t value)} umożliwia zapis wartości do rejestru wskazanego typu. W przypadku rejestrów złożonych zapis realizowany jest poprzez bezpośrednią modyfikację odpowiednich par rejestrów 8-bitowych. Mechanizm ten zapewnia spójność reprezentacji stanu procesora oraz minimalizuje koszt operacji zapisu.

\paragraph{Operacje na rejestrach 8-bitowych}

Dodatkowe funkcje \texttt{cpu\_read\_reg8} oraz \texttt{cpu\_set\_reg8} umożliwiają bezpośredni dostęp do rejestrów bajtowych. Szczególnym przypadkiem jest rejestr logiczny wskazujący adres pamięci (\texttt{HL}), dla którego odczyt i zapis realizowane są poprzez operacje magistrali systemowej.

\paragraph{Zarządzanie flagami procesora}

Funkcja \texttt{cpu\_set\_flags} odpowiada za kontrolowaną modyfikację bitów statusowych. Wprowadzenie wartości specjalnej oznaczającej brak modyfikacji pozwala instrukcjom zmieniać jedynie wybrane flagi bez ingerencji w pozostałe. Na końcu operacji maskowane są młodsze bity rejestru flag, co zapewnia zgodność z architekturą procesora.

\subsubsection{Procedury wykonawcze instrukcji}

Wykonanie instrukcji realizowane jest poprzez zestaw wyspecjalizowanych procedur przypisanych do typów operacji. Mechanizm wyboru procedury oparty jest na tablicy wskaźników funkcji, co umożliwia szybkie mapowanie typu instrukcji na jej implementację.

\paragraph{Operacje arytmetyczne i logiczne}

Instrukcje takie jak dodawanie, odejmowanie, operacje logiczne oraz porównania manipulują rejestrami procesora oraz odpowiednio aktualizują flagi statusowe. Implementacja uwzględnia rozróżnienie operacji 8-bitowych i 16-bitowych, a także szczególne przypadki wymagane przez specyfikację architektury.

\paragraph{Operacje przepływu sterowania}

Instrukcje skoków, wywołań procedur oraz powrotów wykorzystują mechanizm warunków wykonania oparty na stanie flag procesora. W przypadku wywołań podprogramów wykorzystywany jest stos procesora, który przechowuje adres powrotu.

\paragraph{Instrukcje bitowe i rotacyjne}

Rozkazy prefiksowane implementują operacje manipulacji pojedynczymi bitami, przesunięcia oraz rotacje. Procedury te działają zarówno na rejestrach, jak i na danych znajdujących się w pamięci wskazywanej przez rejestry adresowe.

\paragraph{Instrukcje systemowe}

Instrukcje sterujące stanem procesora obejmują operacje zarządzania przerwaniami, zatrzymania procesora oraz operacje specjalne związane z reprezentacją danych dziesiętnych.

\subsubsection{Mechanizm pobierania operandów}

Pobieranie operandów realizowane jest przez funkcję \texttt{fetch\_data()}, która interpretuje tryb adresowania aktualnej instrukcji i przygotowuje dane wejściowe dla etapu wykonania.

\paragraph{Tryby rejestrowe}

W najprostszym przypadku operand pobierany jest bezpośrednio z rejestru wskazanego w opisie instrukcji.

\paragraph{Tryby natychmiastowe}

W trybach z danymi natychmiastowymi operand odczytywany jest z pamięci programu pod adresem wskazywanym przez licznik rozkazów, który następnie zostaje odpowiednio zwiększony.

\paragraph{Tryby pamięciowe}

Instrukcja może wskazywać adres pamięci jako źródło lub miejsce docelowe operacji. W takim przypadku mechanizm pobierania zapisuje adres docelowy oraz oznacza operację jako zapis do pamięci.

\paragraph{Tryby z autoinkrementacją i autodekrementacją}

Specjalne tryby adresowania wykorzystujące rejestr adresowy umożliwiają automatyczną modyfikację jego wartości po operacji odczytu lub zapisu. Mechanizm ten jest wykorzystywany w operacjach blokowych oraz przy pracy ze strukturami danych w pamięci.

\paragraph{Znaczenie warstwy pobierania danych}

Rozdzielenie etapu pobierania operandów od etapu wykonania instrukcji upraszcza implementację logiki procesora, umożliwia ponowne wykorzystanie kodu oraz zapewnia jednoznaczne odwzorowanie semantyki trybów adresowania w architekturze emulatora.

\subsubsection{Wykonanie w środowisku wielowątkowym}

Funkcja \texttt{cpu\_run\_threaded()} realizuje ciągłą pracę procesora w pętli wykonawczej działającej w osobnym wątku. Funkcja:
\begin{itemize}
	\item inicjalizuje komponenty systemowe,
	\item wykonuje kolejne kroki procesora,
	\item zbiera statystyki wydajności,
	\item synchronizuje stan z modułem graficznym.
\end{itemize}
...
Zastosowana organizacja procesora opiera się na jednoznacznym rozdzieleniu:
\begin{itemize}
	\item stanu architektonicznego,
	\item mechanizmu dekodowania instrukcji,
	\item mechanizmu wykonania,
	\item symulacji czasu systemowego.
\end{itemize}

Takie podejście zapewnia wysoką czytelność implementacji, umożliwia precyzyjne odwzorowanie działania sprzętu oraz stanowi solidną podstawę do dalszej rozbudowy emulatora.
