...
\subsection{Implementacja ROM Only kart'a}

Każdy wydany kartridż do Game Boy'a zawierał tak zwany nagłówek który zawierał informacje o tytule gry, kodzie producenta, jakim typem kartridża jest i rozmiarach pamięci ROM i RAM jeśli owy kart zawiera. Na potrzeby tego projektu, przyjmiemy że obsługiejemy tylko ROMy kartidźy które nie mają żadnych dodatkowych kompomentów (ROM Only).

\begin{figure}[htbp]
	\centering
	\begin{minipage}{0.9\textwidth}
		\centering
		\includegraphics[width=0.9\linewidth]{rom_header_code.png}
		\caption{Struktura nagłówka }
		\label{fig:code_rom_header}
	\end{minipage}
\end{figure}
\begin{itemize}
\item \textit{}
