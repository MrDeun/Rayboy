...
Procesorem Gameboya był specjalnie wytworzyny na potrzeby konsoli  8-bitowy procesor SM83 firmy Sharp. \cite{gbctr}. Procesor był wzorowany na dwóch innych procesorach- 8080 firmy Intel i Z80 firmy Zilog. Procesor znajdował się na płytce wraz innymi komponentami takimi jak pamięć RAM i ROM. Cała płytka była określana jako SoC (System on Chip) i oryginalny Game Boy zawierał DMG-CPU znany także jako Sharp LR35902 \cite{copetti}. SoC był wpinany do płyty głównej, która zawierała dodatkową pamięć RAM i Video RAM (VRAM) . Płyta główna łączyła także inne kompomenty jak ekran, głośnik czy kontroler.
...
\subsubsection{Pamięć programu i przełączanie banków ROM}
...
Generowanie obrazu w konsoli Game Boy jest realizowane przez układ graficzny (PPU - pixel processing unit), który współpracuje z pamięcią wideo oraz pamięcią atrybutów obiektów. Proces ten przebiega cyklicznie i jest ściśle zsynchronizowany z zegarem systemowym oraz działaniem procesora. Obraz powstaje poprzez sekwencyjne renderowanie tła, a następnie nakładanie obiektów ruchomych zapisanych w pamięci OAM (Rozdział \ref{oam}).

Dostęp CPU do pamięci VRAM oraz OAM jest \textbf{synchroniczny z trybem pracy PPU}
-- nie jest możliwe swobodne odczytywanie i zapisywanie tych obszarów pamięci
w dowolnym momencie. W trybie~3 (Drawing) PPU aktywnie odczytuje dane z VRAM
w celu generowania pikseli, dlatego równoległy dostęp CPU do tej pamięci jest
blokowany przez sprzęt. Próba odczytu VRAM przez CPU w tym czasie zwraca wartość
\texttt{0xFF}, natomiast zapis jest ignorowany.

Prawidłowy zapis danych graficznych powinien odbywać się wyłącznie w trybach
dopuszczających dostęp CPU, to znaczy w okresach HBlank (tryb~0) lub VBlank
(tryb~1). Jest to szczególnie istotne przy implementacji procedur aktualizacji
tilesetu lub mapy tła w czasie rzeczywistym.
...
Zastosowanie mechanizmu bezpośredniego transferu bajtów umożliwia szybkie aktualizowanie danych obiektów, co jest szczególnie istotne w systemach czasu rzeczywistego, gdzie konieczna jest synchronizacja zmian graficznych z generowaniem kolejnych klatek obrazu. Po pobraniu 160 bajtów, operacja DMA się zamyka i można znowu korzystać obszaru pamięci
...
\begin{lstlisting}[style=cpp, label={lst:rom_header}, caption={Struktura \texttt{rom\_header}}]
	struct rom_header {
		...
		uint8_t  sgb_flag;            // flaga zgodnosci ze dodatkiem Super Game Boy
...
Nagłówek znajduje się w pamięci ROM pod stałym adresem \texttt{0x100}. W implementacji dostęp do niego realizowany jest poprzez rzutowanie odpowiedniego fragmentu bufora pamięci na strukturę \texttt{rom\_header}. Takie podejście umożliwia bezpośredni odczyt pól nagłówka bez konieczności ręcznej interpretacji poszczególnych bajtów. Ze względu na jednorazowe użycie struktury nie musimy przejmować się pakowaniem obiektów.

\paragraph{Kod regionu (\texttt{dest\_code})}
Pole \texttt{dest\_code} wskazuje region dystrybucji kartridża. Wartość \texttt{0x00}
oznacza rynek japoński, natomiast \texttt{0x01} -- rynki poza Japonią (Europa,
Ameryka Północna). Pole to nie wpływa na działanie emulatora, lecz jest
weryfikowane przez oryginalny firmware konsoli.

\paragraph{Suma kontrolna nagłówka (\texttt{checksum})}
Pole \texttt{checksum} przechowuje sumę kontrolną bajtów nagłówka w zakresie
adresów \texttt{0x134}--\texttt{0x14C}. Oryginalny boot ROM konsoli weryfikuje
tę wartość przed uruchomieniem programu -- niezgodność sumy kontrolnej skutkuje
zatrzymaniem wykonania. Algorytm obliczania sumy kontrolnej przedstawia
listing~\ref{lst:checksum}.

\begin{lstlisting}[style=cpp, label={lst:checksum},
	caption={Weryfikacja sumy kontrolnej nagłówka}]
	uint8_t compute_checksum(const uint8_t* rom) {
		uint8_t checksum = 0;
	...
	bool verify_checksum(const uint8_t* rom) {
		return compute_checksum(rom) == rom[0x14D];
	}
\end{lstlisting}
...
Moduł udostępnia interfejs umożliwiający dostęp do danych pamięci ROM przez pozostałe komponenty emulatora. Funkcja \texttt{cart\_read(uint16\_t address)} zwraca wartość bajtu znajdującego się pod wskazanym adresem pamięci kartridża. Dodatkowo moduł udostępnia funkcję \texttt{get\_rom\_header()}, która zwraca adres nagłówka, umożliwiającą odczyt informacji nagłówkowych przez inne podsystemy emulatora.
...
Poprawność nagłówka weryfikowana jest poprzez obliczenie sumy kontrolnej na podstawie danych znajdujących się w zakresie adresów \texttt{0x0134--0x014C}. Suma kontrolna daje nam pewność że operujemy na prawidłowym ROMie gry. Obliczona wartość porównywana jest z sumą kontrolną zapisaną w nagłówku ROM. Wynik weryfikacji prezentowany jest w postaci komunikatu diagnostycznego podczas inicjalizacji kartridża.
...
Mapowanie przestrzeni adresowej jest opisana wcześniej w tabeli \ref{tab:gb_memory_map}

W przypadku trwającego transferu DMA dostęp do pamięci OAM jest blokowany, a magistrala zwraca 0xFF. Takie zachowanie odwzorowuje ograniczenia sprzętowe rzeczywistego systemu.
...
\begin{itemize}
	\item \texttt{bus\_read16(uint16\_t address)} — odczyt dwóch kolejnych bajtów i ich złożenie w słowo 16-bitowe,
	\item \texttt{bus\_write16(uint16\_t address, uint16\_t value)} — zapis wartości 16-bitowej w postaci dwóch bajtów.
\end{itemize}
...
Podstawową jednostką działania procesora jest \textbf{cykl wykonawczy} (ang.
\textit{fetch-decode-execute cycle}) -- sekwencja trzech kroków realizowanych
przez CPU dla każdej instrukcji. W pierwszym kroku (\textit{fetch}) procesor
pobiera kod instrukcji spod adresu wskazywanego przez licznik rozkazów
\texttt{PC}. Następnie w kroku \textit{decode} jednostka sterująca rozpoznaje
rodzaj operacji i określa jej operandy. W ostatnim kroku (\textit{execute})
instrukcja jest wykonywana, a licznik \texttt{PC} zostaje zaktualizowany.
W procesorze Sharp SM83 zastosowanym w Game Boyu czas trwania cyklu wykonawczego
wyrażany jest w \textbf{maszynowych cyklach zegarowych} (ang. \textit{machine
	cycles}), gdzie jeden cykl maszynowy odpowiada 4~taktom zegara systemowego.

Implementacja CPU opiera się na  strukturze kontekstu procesora (listing \ref{lst:cpu_context}), przechowującej pełny stan procesora oraz dane pomocnicze wykorzystywane podczas dekodowania i realizacji instrukcji.
...
\begin{lstlisting}[style=cpp,caption={Kontekst wykonania procesora},label={lst:cpu_context}]
	struct cpu_context {
		cpu_registers regs;
...
Klasa \texttt{RayboyUI} zarządza wszystkimi elementami graficznymi interfejsu użytkownika (rysunek \ref{fig:rayboy_ui}). 
...
\subsection{Sterowanie w trakcji działania emulatora}\label{controls}
...
\subsection{Działanie emulatora}

Po uruchomieniu programu emulator inicjalizuje kolejno podsystemy: wczytuje obraz
ROM z podanej ścieżki, weryfikuje sumę kontrolną nagłówka, a następnie uruchamia
wątek CPU oraz interfejs graficzny. Od tego momentu emulacja przebiega w czasie
rzeczywistym -- wątek CPU wykonuje instrukcje procesora SM83 z częstotliwością
odpowiadającą oryginalnej konsoli, natomiast wątek główny odpowiada za renderowanie
obrazu i obsługę wejścia użytkownika.

\paragraph{Ekran główny}
Podstawowy widok emulatora przedstawia obraz generowany przez PPU w rozdzielczości
160$\times$144~piksele, przeskalowany do rozmiarów okna. W prawym górnym rogu
wyświetlane są podstawowe informacje diagnostyczne: aktualny tryb pracy PPU oraz
liczba klatek na sekundę (FPS).

\paragraph{Widok kafelków (F1)}
Naciśnięcie klawisza \texttt{F1} przełącza widok na podgląd pamięci kafelków
(ang. \textit{tileset viewer}). Wyświetlana jest zawartość obszaru VRAM
zawierającego dane graficzne aktualnie załadowanych kafelków tła i obiektów.
Widok ten jest przydatny podczas debugowania -- umożliwia weryfikację poprawności
wczytanych danych graficznych bez konieczności uruchamiania zewnętrznych narzędzi.

\paragraph{Widok statystyk (F2)}
Klawisz \texttt{F2} przełącza nakładkę ze statystykami emulatora, która
prezentuje szczegółowe informacje o stanie wewnętrznym systemu: wartości rejestrów
procesora (\texttt{AF}, \texttt{BC}, \texttt{DE}, \texttt{HL}, \texttt{PC},
\texttt{SP}), aktualny tryb pracy PPU, stan flag procesora oraz liczbę
wykonanych cykli maszynowych.

\paragraph{Wstrzymanie emulacji (Spacja)}
Naciśnięcie klawisza \texttt{Spacja} wstrzymuje lub wznawia wykonanie wątku CPU.
W trybie wstrzymania interfejs graficzny pozostaje aktywny -- możliwe jest
przeglądanie widoków diagnostycznych bez wpływu na stan emulatora. 
\paragraph{Ekran pomocy (H)}
Klawisz \texttt{H} wyświetla nakładkę z podglądem dostępnych skrótów klawiszowych,
opisanych w sekcji~\ref{controls}.
\newpage
\section{Zakończenie}
