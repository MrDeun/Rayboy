...
Podstawowym zestawem testów wykorzystanym w procesie weryfikacji były testy autorstwa \textit{blargg}. Zestaw ten jest szeroko stosowany w społeczności twórców emulatorów i służy do sprawdzania poprawności implementacji procesora oraz wybranych elementów sprzętowych konsoli. Testy te weryfikują między innymi działanie instrukcji procesora, obsługę przerwań oraz funkcjonowanie timerów.
...
\begin{figure}[htbp]
	...
	\begin{minipage}{0.45\textwidth}
		...
		\caption{Wynik przechodzącego poprawnie testu \textit{01-special.gb}\cite{copetti}}
		\label{fig:succesful_blaarg}
	\end{minipage}
\end{figure}


W przypadku niepowodzenia testów przeprowadzana jest dodatkowa analiza przy użyciu narzędzia \textit{gameboy-doctor}. Emulator generuje plik zawierający szczegółowe logi wykonywania programu, obejmujące stan procesora dla każdego cyklu pracy, w tym wartości rejestrów oraz aktualnie wykonywaną instrukcję.
\begin{figure}[htbp]
	\centering
	\begin{minipage}{0.45\textwidth}
		\centering
		\includegraphics[width=0.9\linewidth]{failed-blaarg.png}
		\caption{Zrzut terminalu z źle zaimplementowanej instrukcji\cite{copetti}}
	...
	\begin{minipage}{0.45\textwidth}
		...
		\caption{Przykład diagnozy w którym\cite{copetti}}
...
Ostatnim etapem weryfikacji było porównanie działania emulatora z dojrzałą implementacją referencyjną. W tym celu wykorzystano emulator \textit{SameBoy}, który jest uznawany za jedną z najbardziej dokładnych implementacji emulatora konsoli Game Boy. 
...
Ze względu na bardzo specficzne implementację tabeli op-codów, nie wszytkie kompilatory C++ wspierają daną składnię - aktualnie tylko kompilator \cite{clang++} wspiera nieoznaczone inicjalizatory. 
...
\subsubsection{Sterowanie w trakcji działania emulatora}
